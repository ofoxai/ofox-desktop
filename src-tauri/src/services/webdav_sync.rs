...
const PROTOCOL_FORMAT: &str = "ofox-switch-webdav-sync";
...
mod tests {
    ...
    fn remote_dir_segments_uses_current_layout() {
        let settings = WebDavSyncSettings {
            remote_root: "ofox-switch-sync".to_string(),
            profile: "default".to_string(),
            ..WebDavSyncSettings::default()
        };
        let segs = remote_dir_segments(&settings, RemoteLayout::Current);
        assert_eq!(segs, vec!["ofox-switch-sync", "v2", "db-v6", "default"]);
    }
    ...
    fn remote_dir_segments_uses_legacy_layout() {
        ...
        assert_eq!(segs, vec!["ofox-switch-sync", "v2", "default"]);
    }
