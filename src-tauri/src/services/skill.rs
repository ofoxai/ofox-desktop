...
impl SkillService {
    ...
    /// 扫描未管理的 Skills
    ///
    /// 扫描各应用目录，找出未被 Ofox Switch 管理的 Skills
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "ofox-switch".to_string()));
        }
    ...
    /// 从应用目录导入 Skills
    ///
    /// 将未管理的 Skills 导入到 Ofox Switch 统一管理
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "ofox-switch".to_string()));
...
mod tests {
    ...
    fn get_app_skills_dir_honors_test_home_override() {
        ...
        env.set("CC_SWITCH_TEST_HOME", temp.path());

        let dir = SkillService::get_app_skills_dir(&AppType::OpenCode).expect("resolve skills dir");

        assert_eq!(dir, temp.path().join(".config/opencode/skills"));
    }
}
